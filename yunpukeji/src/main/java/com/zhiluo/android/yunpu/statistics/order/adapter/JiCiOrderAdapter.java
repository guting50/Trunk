...
public class JiCiOrderAdapter extends RecyclerView.Adapter<JiCiOrderAdapter.Myholder> {
    private Context mContext;
    private TimesConsumeOrderBean bean;
    //private double money = 0;
    private OnItemClickListener itemClickListener;
    private String name = "",cardnum = "";


    public JiCiOrderAdapter(Context mContext, TimesConsumeOrderBean bean, OnItemClickListener itemClick) {
    ...
    public void onBindViewHolder(Myholder holder, final int position) {
        String time = bean.getData().getDataList().get(position).getWO_UpdateTime();//时间
        ...
        double eachmoney = 0;
        double dayMoney = 0;
        for (int i = 0; i < bean.getData().getDataList().get(position).getWouldOrderDetail().size(); i++) {
            eachmoney += bean.getData().getDataList().get(position).getWouldOrderDetail().get(i).getSG_Price() * bean.getData().getDataList().get(position).getWouldOrderDetail().get(i).getWOD_UseNumber();
        }
    ...
    private double getMoney(int position) {
        ...
        timenow = bean.getData().getDataList().get(position).getWO_UpdateTime();
        timelast = bean.getData().getDataList().get(position - 1).getWO_UpdateTime();
        if (timenow.substring(0, 10).equals(timelast.substring(0, 10))) {
            daymoney += getItemMoney(bean, position);
        } else {
        }
        return daymoney;
    }
     ...
     * @param bean     ,
     * @param position ,当前位置
     * @return 获取每一项中的子集总的钱数；
     */
    private double getItemMoney(TimesConsumeOrderBean bean, int position) {
        double itemmoney = 0;
        for (int i = 0; i < bean.getData().getDataList().get(position).getWouldOrderDetail().size(); i++) {
            itemmoney += bean.getData().getDataList().get(position).getWouldOrderDetail().get(i).getSG_Price() * bean.getData().getDataList().get(position).getWouldOrderDetail().get(i).getWOD_UseNumber();
        }
        return itemmoney;
    }


    private boolean isVisiliable(TimesConsumeOrderBean bean, int position) {
    ...
    public void setParam(TimesConsumeOrderBean mReportBean) {
        bean = mReportBean;
    }
