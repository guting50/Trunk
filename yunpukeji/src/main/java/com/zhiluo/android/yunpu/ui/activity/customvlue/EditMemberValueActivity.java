...
public class EditMemberValueActivity extends BaseActivity {
    ...
    TextView tvSave;
    @Bind(R.id.tv_edit_member_value_type)
    TextView tvType;
    @Bind(R.id.et_edit_member_value_name)
    EditText etName;
    @Bind(R.id.et_edit_member_value_sort)
    EditText etSort;
    @Bind(R.id.rb_edit_member_value_no_empty)
    RadioButton rbNoEmpty;
    @Bind(R.id.rb_edit_member_value_empty)
    RadioButton rbEmpty;
    @Bind(R.id.rb_edit_member_value_show)
    RadioButton rbShow;
    @Bind(R.id.rb_edit_member_value_hide)
    RadioButton rbHide;
    @Bind(R.id.et_edit_member_data)
    EditText etData;
    ...
    private boolean verify() {
        if (TextUtils.isEmpty(etName.getText())) {
            CustomToast.makeText(EditMemberValueActivity.this, "属性名称不能为空！", Toast.LENGTH_SHORT).show();
            return false;
        }
        if (TextUtils.isEmpty(etSort.getText())) {
            CustomToast.makeText(EditMemberValueActivity.this, "属性排序不能为空！", Toast.LENGTH_SHORT).show();
            return false;
        }
        if (mValueBean.getCF_FieldType().equals("选项")) {
            if (TextUtils.isEmpty(etData.getText())) {
                CustomToast.makeText(EditMemberValueActivity.this, "数据集不能为空！", Toast.LENGTH_SHORT).show();
                return false;
            }
        }
        return true;
    ...
    private void edit() {
        ...
        CallBack callBack = new CallBack() {
            ...
            public void onSuccess(String responseString, Gson gson) {
                mSweetAlertDialog = new SweetAlertDialog(EditMemberValueActivity.this, SweetAlertDialog.SUCCESS_TYPE);
            ...
            public void onFailure(String msg) {
                CustomToast.makeText(EditMemberValueActivity.this, msg, Toast.LENGTH_SHORT).show();
            }
        };
        callBack.setLoadingAnimation(this, "提交中...", false);
        HttpHelper.post(this, "CustomFieldsVIP/EditFieldsVIP", params, callBack);
    }
