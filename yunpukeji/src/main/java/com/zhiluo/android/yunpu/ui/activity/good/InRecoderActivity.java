...
public class InRecoderActivity extends AppCompatActivity implements InRecoderAdapter.OnItemClick, View.OnClickListener {
    ...
    private TextView mTitledates, mStartDates, mEndDates,tvnull;
        ...
        mRefresh.setOnRefreshListener(new WaveSwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onLoad() {
                if (mRefreshIndex <= mPageTotal) {
                    ...
                    CustomToast.makeText(InRecoderActivity.this, "没有更多数据了", Toast.LENGTH_SHORT).show();
...
//        DataUtils.getDateBefore(15).substring(0, 10)
...
//        getGoods();
    ...
    public void itemClick(View view, int position) {
        mBundle = new Bundle();
        mIntent = new Intent(this, InWareItemDetailActivity.class);
        mBundle.putSerializable("Bean", (Serializable) inRecoderBean);
    ...
    public void getGoods() {
        ...
        HttpHelper.post(this, HttpAPI.API().SERVICEGOODS, params, new CallBack() {
            ...
            public void onFailure(String msg) {
                CustomToast.makeText(InRecoderActivity.this, msg, Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void obtainData(int PageIndex, int PageSize) {
        ...
        CallBack callBack = new CallBack() {
            @Override
            public void onSuccess(String responseString, Gson gson) {
                InRecoderBean entity = CommonFun.JsonToObj(responseString, InRecoderBean.class);
                mPageTotal = entity.getData().getPageTotal();
                number = 0;
                if (inRecoderBean != null && mIsLoadMore) {
                    inRecoderBean.addAll(entity.getData().getDataList());
                } else {
                    inRecoderBean = entity.getData().getDataList();
                }


                for (int i = 0; i < inRecoderBean.size(); i++) {
                    number += inRecoderBean.get(i).getSI_TotalPrice();
                }
                ...
                mRefresh.setLoading(false);
                mIsLoadMore = false;
                if (inRecoderBean.size()>0){
                    tvnull.setVisibility(View.GONE);
                }else {
                    tvnull.setVisibility(View.VISIBLE);
                }
                LinearLayoutManager manager = new LinearLayoutManager(InRecoderActivity.this);
                manager.setOrientation(OrientationHelper.VERTICAL);

                if (adapter == null){
                    adapter = new InRecoderAdapter(InRecoderActivity.this, inRecoderBean, InRecoderActivity.this);
                    recyclerView.setLayoutManager(manager);
                    recyclerView.setAdapter(adapter);
                }else {
                    adapter.setParm(inRecoderBean);
        ...
        HttpHelper.post(this, "StockManager/GetStockInDataList", params, callBack);
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_date_confirm:
                if (mStartDates.getText().toString().isEmpty()) {
                    ...
                    if ( !DateTimeUtil.isCurTime(InRecoderActivity.this,mStartTime)){
                        return;
                    }else if (!DateTimeUtil.isCurTime(InRecoderActivity.this,mEndTime)){
                        return;
                    }
                    try {
                        if (DateTimeUtil.isOverTime(InRecoderActivity.this, mStartTime, mEndTime)) {
    ...
    private void showDateDialog(List<Integer> date, final TextView textViews,final int tag) {
        ...
        builder.setOnDateSelectedListener(new DatePickerDialog.OnDateSelectedListener() {
            ...
            public void onDateSelected(int[] dates) {
                ...
                if (tag ==0){
                    mStartTime = str;
                    DateTimeUtil.isCurTime(InRecoderActivity.this,mStartTime);
                }else if (tag ==1){
                    mEndTime = str;
                    DateTimeUtil.isCurTime(InRecoderActivity.this,mEndTime);
                }
            }
