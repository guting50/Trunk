 ...
 */
public class GoodsRecoderAdapter extends RecyclerView.Adapter<GoodsRecoderAdapter.Myholder> {
    private Context mContext;
    private GoodsOrderReportBean bean;
    //private double money = 0;
    private OnItemClickListener itemClickListener;
    private String name = "",cardnum= "";


    public GoodsRecoderAdapter(Context mContext, GoodsOrderReportBean bean, OnItemClickListener itemClick) {
    ...
    public void onBindViewHolder(Myholder holder, final int position) {
        ...
        for (int i = 0; i < bean.getData().getDataList().size(); i++) {
            eachmoney += bean.getData().getDataList().get(position).getCO_Monetary();
        }
        holder.tvDiscountMoney.setText(Decima2KeeplUtil.stringToDecimal(bean.getData().getDataList().get(position).getCO_TotalPrice() + ""));
        ...
        if (position < bean.getData().getDataList().size()) {
            ...
            }else if (bean.getData().getDataList().get(position).getCO_ConsumeWay().contains("其它")){
    ...
    private boolean isVisiliable(GoodsOrderReportBean bean, int position) {
    ...
    public void setParm(GoodsOrderReportBean mReportBean) {
        bean =mReportBean ;
    }
