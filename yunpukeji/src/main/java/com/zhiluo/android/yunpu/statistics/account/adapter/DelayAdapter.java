...
public class DelayAdapter extends BaseAdapter {
    Context context;
    DelayReportBean reportBean;

    public DelayAdapter(Context context, DelayReportBean reportBean) {
    ...
    public View getView(int i, View convertView, ViewGroup viewGroup) {
        DelayReportBean.DataBean.DataListBean bean = reportBean.getData().getDataList().get(i);
        ...
        holder.tvOrder.setText(bean.getMDR_Order());
        if (bean.getVIP_Name()!=null){
            holder.tvName.setText(bean.getVIP_Name());
        }else {
            holder.tvName.setText(bean.getVIP_Card());
        }
        holder.tvValue1Name.setText("延期时长：");
        holder.tvValue2Name.setText("延期费用：");
        holder.tvValue1.setText(bean.getMDR_Time());
        holder.tvValue2.setText(bean.getMDR_Money()+"");
        return view;
    }
