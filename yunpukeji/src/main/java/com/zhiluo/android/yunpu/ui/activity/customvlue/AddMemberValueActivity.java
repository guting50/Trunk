 ...
 * 新增会员自定义属性
 */
public class AddMemberValueActivity extends BaseActivity {

    @Bind(R.id.tv_back_activity)
    TextView tvBack;
    @Bind(R.id.tv_add_user_save)
    TextView tvSave;
    @Bind(R.id.et_add_member_value_name)
    EditText etName;
    @Bind(R.id.et_add_member_value_sort)
    EditText etSort;
    @Bind(R.id.rb_add_member_value_no_empty)
    RadioButton rbNoEmpty;
    @Bind(R.id.rb_add_member_value_empty)
    RadioButton rbEmpty;
    @Bind(R.id.rg_add_user_sex)
    RadioGroup rgSex;
    @Bind(R.id.rb_add_member_value_show)
    RadioButton rbShow;
    @Bind(R.id.rb_add_member_value_hide)
    RadioButton rbHide;
    @Bind(R.id.et_add_member_data)
    EditText etData;
    @Bind(R.id.tv_shoise_type)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_member_value);
    ...
    private boolean verify() {
        ...
        if (TextUtils.isEmpty(etName.getText())) {
            CustomToast.makeText(AddMemberValueActivity.this, "属性名称不能为空！", Toast.LENGTH_SHORT).show();
            return false;
        }
        if (TextUtils.isEmpty(etSort.getText())) {
            CustomToast.makeText(AddMemberValueActivity.this, "属性排序不能为空！", Toast.LENGTH_SHORT).show();
            return false;
        }
        if (flags == 2) {
            if (TextUtils.isEmpty(etData.getText())) {
                CustomToast.makeText(AddMemberValueActivity.this, "数据集不能为空！", Toast.LENGTH_SHORT).show();
                return false;
            }
        }
        return true;
    ...
    private void add() {
        ...
        params.put("CF_IsShowVIP", rbShow.isChecked() ? 1 : 0);
        ...
        CallBack callBack = new CallBack() {
            ...
            public void onSuccess(String responseString, Gson gson) {
                mSweetAlertDialog = new SweetAlertDialog(AddMemberValueActivity.this, SweetAlertDialog.SUCCESS_TYPE);
            ...
            public void onFailure(String msg) {
                CustomToast.makeText(AddMemberValueActivity.this, msg, Toast.LENGTH_SHORT).show();
            }
        };
        callBack.setLoadingAnimation(this, "提交中...", false);
        HttpHelper.post(this, "CustomFieldsVIP/AddFieldsVIP", params, callBack);
    }
